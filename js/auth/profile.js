...
inputImage.addEventListener('change', () => {
    let t = "image/" + (inputImage.files[0].name);
    updateImage(t);
});
...
function placeImage(userData){
    ...
    if(userData.image== "" || userData.image === null){
        return `<img id="profile-image" src="https://eu.ui-avatars.com/api/?name=${userData.name}&size=250" alt="profileImage">`
    } else{
        return `<img id="profile-image" src="${userData.image}" alt="profileImage">`
    }
};
